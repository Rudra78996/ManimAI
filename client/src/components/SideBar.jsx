import { Plus, Search, BookOpen, Bot, Blocks, MessageSquare, Zap, ChevronRight, X as LucideX } from "lucide-react";
...
const AppSidebar = ({ isOpen, toggleSidebar }) => {
  ...
  const chats = [
    ...
    "Time Complexity Subsequences",
    "Message Bubble Layout Fix",
    "Manim API Animation Prompt",
    "Mongoose validation error fix",
    "Message Section Optimization",
    "500 Error Analysis",
    "Possible k Values",
    "NPTEL Local Chapter Query",
    "Inches to Centimeters Conversion",
    "Socket.io Client Issue",
  ];

  if (!isOpen) {
    return (
      <div className="flex flex-col h-screen w-0 min-w-0 relative">
        <Button
          variant="outline"
          size="icon"
          ...
          style={{ outline: "none", width: 32, height: 32, minWidth: 32, minHeight: 32 }}
        >
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 px-4 py-3 border-b border-sidebar">
        ...
        <h1 className="text-xl font-semibold text-sidebar-foreground flex-1">ManimAI</h1>
        <Button
          variant="outline"
          size="icon"
      ...
      <div className="px-3 py-4 space-y-2">
        <Button className="w-full flex items-center gap-3" size="default">
      ...
      <nav className="px-2 py-2">
        {[
          { icon: <BookOpen className="w-4 h-4" />, label: "Library" },
          { icon: <Blocks className="w-4 h-4" />, label: "GPTs" },
        ].map((item, index) => (
          <motion.a
            key={index}
            whileHover={{ x: 2 }}
      ...
      <div className="flex-1 overflow-y-auto px-2 pb-4 space-y-1">
        {chats.map((chat, index) => (
          <motion.a
            ...
            href="#"
            className="block px-3 py-2.5 rounded-lg text-sm transition-colors text-sidebar-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground"
          >
